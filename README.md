...
A collection of database health check tools that collect diagnostic data, produce structured CSV output, and surface actionable findings.
...
|---|---|---|
| [MSSQL/](MSSQL/) | SQL Server | [SQL-Server-Health-Checks](https://github.com/MariosPavlidis/SQL-Server-Health-Checks) |

---

## MSSQL — SQL Server Health Check

Covers 22 chapters. Each chapter maps to a PowerShell script (`powershell/NN_*.ps1`) and one or more T-SQL scripts (`sql/NN_*/`). Run via orchestrator or per-chapter. See [HOW-TO.md](HOW-TO.md) for full usage instructions.
...
Output is written to `MSSQL\powershell\output\<timestamp>_<instance>\`.
...
Each chapter produces one CSV per check section under:

```
MSSQL\powershell\output\<timestamp>_<instance>\
```

All CSVs include `CollectedAt` (UTC) and `SqlInstance` metadata columns. Many scripts include `flag_*` columns (0/1) — filter on `flag_* = 1` to surface items needing attention.

See [HOW-TO.md](HOW-TO.md) for permissions reference, troubleshooting, and version guards.
